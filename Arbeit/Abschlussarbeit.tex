%% Standard-Vorspann
% je nach druckart oneside, twoside
\documentclass[a4paper,11pt,twoside]{report}
\usepackage{ngerman}  
\usepackage[utf8]{inputenc}                
%\usepackage[latin1]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[fleqn]{amsmath}
\usepackage{mathtools}
\usepackage{bm}
\DeclarePairedDelimiter\ceil{\lceil}{\rceil}
\DeclarePairedDelimiter\floor{\lfloor}{\rfloor}

%% Auf Schriftart Palatino umschalten
\usepackage{mathpazo}
\usepackage[scaled=.95]{helvet}
\usepackage{courier}
\usepackage[figuresright]{rotating}
%% fast immer benoetigte Pakete
\usepackage{acronym}
\usepackage{amssymb, amsthm}
\usepackage{alltt}
\usepackage{latexsym}
\usepackage{makeidx}
\usepackage{textcomp}
\usepackage{rotating}
\usepackage{color}
\usepackage[table]{xcolor}
\usepackage{caption}
\DeclareCaptionType{code}[Codeausschnitt][Liste der Codeausschnitte]
\usepackage{listings}
\usepackage{verbatim}
\usepackage{fancyhdr}
\usepackage{graphicx}
\usepackage{subcaption}
\newtheorem{axiom}{Axiom}
\usepackage{tikz}
\usepackage{multirow}
\usepackage[natbib,maxnames=3,style=numeric,backend=bibtex,sortlocale=de]{biblatex}
\addbibresource{bibtex/literature.bib}
\DeclareBibliographyCategory{fullcited}
\usepackage{listings}
\usepackage[hang]{footmisc}
\usepackage{chngcntr}
\counterwithout{footnote}{chapter}
\setlength{\footnotemargin}{0pt}
\usepackage{url}
\renewcommand\UrlFont{\sffamily\slshape}
\usepackage{nicefrac}
\usepackage{units}
\usepackage{minibox}
\pdfminorversion=7
\usepackage{emptypage}
\usepackage{hyperref}
\usepackage{float}
\newfloat{lstfloat}{htbp}{lop}
\usepackage[texindy]{imakeidx}
\usepackage[nonumberlist, numberedsection, toc, section=chapter]{glossaries}  
\renewcommand*{\glspostdescription}{}

%% spezielles Zeug
\usepackage{Abschlussarbeit}
\usepackage{epstopdf}



\usepackage{thmtools}
\declaretheorem[style=Definition, title=Definition, numberwithin=chapter]{dfn}



%% Index erzeugen
\makeindex
\input{chapters/Glossar}

\setcounter{secnumdepth}{4}

%% jetzt geht's los
\begin{document}
\raggedbottom
\hypersetup{
	colorlinks,
	citecolor=black,
	filecolor=black,
	linkcolor=black,
	urlcolor=black
}

\definecolor{mygreen}{rgb}{0,0.6,0}
\definecolor{mygray}{rgb}{0.5,0.5,0.5}
\definecolor{mymauve}{rgb}{0.58,0,0.82}

\lstset{language=C,
		basicstyle=\footnotesize,
		commentstyle=\color{mygreen},
		keywordstyle=\color{blue},
		numbers=left,
		numbersep=5pt,
		numberstyle=\tiny\color{mygray},
		breakatwhitespace=false,
		morekeywords={uint8\_t, spinlock\_t, uint32\_t, bool, NULL, int32\_t, resource\_size\_t,
					  size\_t, u8, \_\_iomem},
		tabsize=2,	
		%frame=single, %todo ja oder nein??
		breaklines=true}
\captionsetup[code]{font=it,
						  position=below,
					  	  belowskip=0.5cm}

%% Die Datei "Vorspann.tex"  bitte ausf�llen!

\include{chapters/Vorspann}  
\include{chapters/Abkuerzungsverzeichnis}
\printglossary[title={Glossar}]

%% Hier kommt der eigentlich Text der Arbeit
%% sinnvoll ist, f�r jedes Kapitel eine eigene 
%% Datei zu nehmen, verk�rzt die Zeit beim LaTeX-Durchlauf
%% Erst am Schluss alle Dateien verwenden.
\newpage
\renewcommand{\thechapter}{\arabic{chapter}} 
\setcounter{chapter}{0}
\pagenumbering{arabic}

\include{chapters/Einleitung}
\include{chapters/Theoretische_Grundlagen}
%\include{chapters/Aktuelle_Implementierung}
\include{chapters/Fachlicher_Hauptteil}
\include{chapters/Test}
\include{chapters/Ausblick}




%% Beim Anhang gibt es eine Musterdatei, die 
%% erg�nzt werden kann
\include{chapters/Anhang}


%% Stichwortverzeichnis ausgeben.
%% falls nicht notwendig, die folgende Zeile loeschen



\end{document}
