...
In der \textit{copy\_from\_user} (siehe Codeausschnitt~\ref{code:code}) werden die Daten ab \textit{from} aus dem Userspace mit der Größe von \textit{num} Bytes an die Stelle \textit{to} in den Kernel kopiert.
...
Auf der Abbildung~\ref{fig:mfd} sieht man die Beziehung zwischen dem Elterngerät (links) und den Kindergeräten (rechts), die jeweils verschiedene Funktionalitäten haben können.
