...
\end{lstlisting}
\captionof{code}{\label{code:res}resource Struktur in \cite[linux/ioport.h, Zeile 20ff.]{linuxsourceinclude}}

%todo resource beschreiben
...
\end{lstlisting}
\captionof{code}{\label{code:mfd_cell}mfd\_cell Struktur in \cite[linux/mfd/core.h, Zeile 29ff.]{linuxsourceinclude}}

%todo *of_compatible nötig??
In der \textit{mfd\_cell} Struktur werden lediglich die benötigten Parameter betrachtet. In der Variable \textit{name} und \textit{id} werden der Name des Treibers und eine spezifische Nummer gespeichert.
Der \textit{void} Zeiger \textit{platform\_data} ist ein benutzerdefinierter Datentyp der an das untergeordnete Gerät weitergereicht wird, da der Typ variieren kann, wird in \textit{pdata\_size} die zugehörige Datengröße übergeben. 

%todo mfd_cell weiter
...
Mithilfe der oberen Funktion legt man die Untergeräte an, zusätzlich ist hier implementiert, dass beim Entfernen des übergeordneten Geräts alle Untergeräte automatisch aufgelöst werden. \cite[mfd/mfd-core.h, Zeile 356f.]{linuxsourcedriver}
...
Durch den Integer \textit{n\_devs} wird die Anzahl der zu registrierenden Kindergeräten angegeben. Dies ist notwendig, da der Parameter \textit{cells} auch ein Array beinhalten kann. Die anderen Übergabeparameter werden nicht näher betrachtet, da sie im folgenden nicht benötigt werden. \cite[mfd/mfd-core.h]{linuxsourcedriver}
