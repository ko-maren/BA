...
Beim Laden wird eine Klasse erstellt und Speicherplatz allokiert. Nach dem Setzen der Major ID in einer Datenstruktur wird eine Minor ID Liste und Spinlock initialisiert. Über die Liste wird später eine freie Minor ID ausgewählt und gesetzt. Der Spinlock sorgt bei diesem Vorgang dafür, dass dieser nicht unterbrochen wird und keine Minor ID doppelt vergeben wird.
Am Ende der Funktion wird der Plattformtreiber mit der platform\_driver Struktur registriert.   
Analog wird beim Freigeben des Modules der Speicherplatz freigegeben, der Plattformtreiber wird abgemeldet und die Klasse zerstört.


Damit das Kernel-Modul ordnungsgemäß geladen bzw. entladen wird, werden hierzu zwei Funktionen erstellt. Durch ein Makro des Linuxkernels (zitat?) werden diese Funktionen, dann anstelle/.. des .. aufgerufen.
