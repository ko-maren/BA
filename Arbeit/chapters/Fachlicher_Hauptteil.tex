...
%todo pid synonym
Als Erstes \ac{ioctl} ist eine Anmeldung der Anwendung bei dem Gerät notwendig. Um die Eindeutigkeit zu garantieren wird hier die pid übergeben. Allerdings wird auch der Prozessname benötigt, damit eine schnelle Nachvollziehbarkeit vorhanden ist. Wie in Kapitel~\ref{sec:konzept} erläutert, soll die Unterscheidung zwischen Standardapplikation und Debugtool möglich sein. Aus diesem Grund wird bei der Anmeldung zusätzlich zur pid und dem Prozessnamen auch der Typ der Applikation übergeben.

\section{Implementierung im Kernel}\label{sec:kernel}
% Anlegen der Devices, Über Ioctl im Arri fpga treiber, öffnen und schließen des device, im eigenen Verzeichnis
