...
This report decribes the improvement of a camera software in interaction with the hardware. For this purpose an abstraction layer will be introduced, which allows access from the software to the modules of the FPGA in a clearer and simpler way. The access to the hardware currently takes place directly via the address of the modules. This should be abstracted, so that the software only needs the name of a module to access the correct module. To facilitate understanding, a brief overview of the current implementation and a short introduction to the image chain are given. Besides, the basics of the Linux operating system and the required components are described. The implementaion of the abstraction layer is then explained in more detail. In the duture the maintainability and the extension of the software shall be facilitated by the abstraction layer.
