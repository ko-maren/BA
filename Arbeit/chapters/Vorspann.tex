...
\vspace*{10mm}
{\huge \bfseries{%
                      \par Modulare Ansteuerung eines FPGA \"uber Software \par
}} 
...
\vspace*{10mm}
{\huge \bfseries{%
		\par Modulare Ansteuerung eines bildverarbeitenden FPGA "uber generische Kernelmodule \par
}} % deuscher Langtitel

\vspace*{10mm}
{\huge \bfseries{%
		\par Modular control of an image processing FPGA via generic kernel modules \par
...
In dem vorliegenden Bericht geht es um die Verbesserung einer Kamerasoftware im Zusammenspiel mit der Hardware. Es wird hierzu eine Abstraktionsebene eingef\"uhrt, welche die Zugriffe von der Software auf die Module des FPGA \"ubersichtlicher und einfacher gestalten soll. Der Zugriff auf die Hardware findet aktuell direkt \"uber die Adresse des Moduls statt und soll abstrahiert werden, sodass die Software lediglich den Modulnamen ben\"otigt, um auf das richtige Modul zuzugreifen. Zum Erleichtern des Verst\"andnisses wird ein kurzer \"Uberblick \"uber die aktuelle Implementierung und eine kurze Einf\"uhrung zur Bildkette gegeben. Au\ss{}erdem wird auf das Betriebssystem Linux und ben\"otigte Komponenten eingegangen. Im Anschluss wird die Implementierung der Abstraktionsebene n\"aher erkl\"art. Die Wartbarkeit und die Erweiterung der Software sollen durch die Abstraktionsebene in Zukunft erleichtert werden.


Dieser Bericht richtet sich an Leser aus dem Bereich der Elektrotechnik.
...
This report decribes the improvement of a camera software in interaction with the hardware. For this purpose an abstraction layer will be introduced, which allows access from the software to the modules of the FPGA in a clearer and simpler way. The access to the hardware currently takes place directly via the address of the modules. This should be abstracted, so that the software only needs the name of a module to access the correct module. To facilitate understanding, a brief overview of the current implementation and a short introduction to the image chain are given. Besides, the basics of the Linux operating system and the required components are described. The implementaion of the abstraction layer is then explained in more detail. In the future the maintainability and the extension of the software shall be facilitated by the abstraction layer.
